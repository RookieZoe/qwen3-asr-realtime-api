...
class SDKCompatibilityTest:
    ...
    def __init__(self, uri="ws://localhost:8001/api-ws/v1/realtime"):
        self.uri = uri
        self.results = []
    ...
    async def test_session_lifecycle(self):
        ...
        try:
            async with websockets.connect(self.uri) as ws:
                ...
                assert data['type'] == 'session.finished', f"Expected session.finished, got {data['type']}"
    ...
    async def test_error_handling(self):
        ...
        try:
            async with websockets.connect(self.uri) as ws:
                ...
                if data['type'] == 'error':
                    self.log(f"✓ Error response received: {data.get('error', {}).get('message', 'Unknown')}")
                    return True
                else:
...
async def main():
    ...
    parser.add_argument('--url', '-u', default='ws://localhost:8001/api-ws/v1/realtime',
                       help='WebSocket URL')
