...
from utils.protocol import (
    generate_session_id, generate_item_id,
    create_session_created_event, create_session_updated_event,
    create_speech_started_event, create_speech_stopped_event,
    create_input_audio_buffer_committed_event,
    create_conversation_item_created_event,
    create_transcription_text_event, create_transcription_completed_event,
    create_transcription_failed_event, create_session_finished_event,
    create_error_event
)
from utils.audio import decode_base64_audio, bytes_to_numpy
...
class WebSocketHandler:
    ...
    async def handle(self):
        ...
        await self._send_event(create_session_created_event(
    ...
    async def _handle_session_update(self, message: Dict[str, Any]):
        ...
        if turn_detection is None:
            ...
            self.vad_silence_ms = turn_detection.get("silence_duration_ms", 400)
        ...
        await self._send_event(create_session_updated_event(
            session_id=self.session_id,
            model=self.model_name,
            input_audio_format=self.audio_format,
            turn_detection={
    ...
    async def _handle_speech_stopped(self, audio_end_ms: int):
        ...
        await self._send_event(create_speech_stopped_event(
    ...
    async def _commit_audio(self):
        ...
        if self.asr_session:
            ...
            logger.info(f"ASR final result: {final_result}")
    ...
    async def _send_transcription_text(self, result: Dict[str, str]):
        ...
        await self._send_event(create_transcription_text_event(
            item_id=self.current_item_id,
            content_index=0,
    ...
    async def _send_error(self, error_type: str, code: str, message: str, 
                         param: Optional[str] = None, event_id: Optional[str] = None):
