...
def decode_pcm_to_numpy(
    audio_bytes: bytes, sample_rate: int = 16000, bits: int = 16
) -> Optional[np.ndarray]:
    try:
        if bits == 16:
            dtype = np.int16
...
def decode_opus_to_numpy(audio_bytes: bytes, sample_rate: int = 16000) -> Optional[np.ndarray]:
    ...
    try:
        ...
        raise ImportError(
            "opuslib is required for Opus audio decoding. Install with: uv sync --extra audio"
        )
    except Exception:
        return None
...
def bytes_to_numpy(
    audio_bytes: bytes, audio_format: str, sample_rate: int = 16000
) -> Optional[np.ndarray]:
