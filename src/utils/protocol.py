...
def create_session_created_event(
    ...
    session = {
        ...
        or {"type": "server_vad", "threshold": 0.5, "silence_duration_ms": 200},
    }
    return create_event("session.created", session=session)


def create_session_updated_event(
    session_id: str,
    model: str,
    input_audio_format: str = "pcm16",
    turn_detection: Optional[Dict] = None,
...
def create_speech_started_event(audio_start_ms: int, item_id: str) -> Dict[str, Any]:
    return create_event(
        "input_audio_buffer.speech_started", audio_start_ms=audio_start_ms, item_id=item_id
    )
...
def create_input_audio_buffer_committed_event(
    ...
    return create_event(
        "input_audio_buffer.committed", previous_item_id=previous_item_id, item_id=item_id
    )
...
def create_transcription_text_event(
    item_id: str, content_index: int, language: str, emotion: str, text: str, stash: str
) -> Dict[str, Any]:
    return create_event(
...
def create_transcription_completed_event(
    item_id: str, content_index: int, language: str, emotion: str, transcript: str
) -> Dict[str, Any]:
    return create_event(
...
def create_transcription_failed_event(
    item_id: str, content_index: int, code: str, message: str, param: str
) -> Dict[str, Any]:
    return create_event(
